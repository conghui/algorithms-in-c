...
static void merge(key_t *c, key_t *a, int M, key_t *b, int N);
inline static int le(key_t a, key_t b);
inline static int comp(const void *l, const void *r);
...
void st_delete(T t, key_t k)
{
    key_t *x = bsearch(&k, t->sorted.data, t->sorted.size,
                       *t->sorted.data, comp);

    if (x != NULL) {
        size_t i = x - t->sorted.data;

        for (; i < t->sorted.size - 1; i++) {
            t->sorted.data[i] = t->sorted.data[i+1];
        }

        t->sorted.size--;
    } else { /* the element is in the unsorted list */
        size_t i;

        for (i = 0; i < t->unsorted.size; i++) {
            if (t->unsorted.data[i] == k) {
                break;
            }
        }

        if (i == t->unsorted.size) {
            return;
        }

        /* the keyfound  */
        for (; i < t->unsorted.size - 1; i++) {
            t->unsorted.data[i] = t->unsorted.data[i+1];
        }
...
inline static int
comp(const void *l, const void *r)
{
    return *(key_t *)l - *(key_t *)r;
}
